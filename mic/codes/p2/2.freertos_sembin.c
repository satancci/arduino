...
void task_one(void *task1_params) {
    while (1) {
        ...
        xSemaphoreTake(semaf_bin, portMAX_DELAY) == pdTRUE ? (
            ESP_LOGI("Task One", "Semaforo adquirido pela tarefa 1!"), xSemaphoreGive(semaf_bin)):
...
void task_two(void *task2_params) {
    while (1) {
        ESP_LOGI("Task Two", "Semaforo adquirido pela tarefa 2!");
...
void app_main(void) {
    ...
    semaf_bin == NULL? (ESP_LOGE("Main", "Falha ao criar o semaf_bin!"), return): 
